...
\begin{document}
    ...
    \begin{abstract}
        \noindent
        Este trabajo aborda la predicción de precios de alquiler en el Área Metropolitana de Buenos Aires (AMBA), utilizando un \emph{dataset} de publicaciones de Mercado Libre. Se desarrolló un \emph{pipeline} completo de preprocesamiento, modelado predictivo y análisis explicativo. La principal contribución es la identificación del impacto de propiedades con precios extremos en las métricas de evaluación, y la propuesta de una arquitectura híbrida que combina un clasificador de segmento de mercado con regresores especializados. Los resultados muestran que el enfoque híbrido logra el menor error absoluto medio (MAE: 23,314) y mediano (MedAE: 934), mejorando notablemente la precisión en el segmento masivo del mercado. Sin embargo, un modelo XGBoost único con hiperparámetros optimizados obtiene el menor error cuadrático medio (RMSE), al capturar mejor los casos extremos. Este \emph{trade-off} entre MAE y RMSE refleja una dualidad del problema: precisión en el segmento promedio vs.\ reducción de errores extremos. Finalmente, mediante análisis SHAP se cuantifica la contribución de cada característica al precio, revelando que superficie, ubicación y amenities premium (gimnasio, pileta) son los principales determinantes, con impactos diferenciales según la zona geográfica.
    \end{abstract}
    ...
    En este trabajo proponemos una alternativa: mantener tanto las propiedades normales como las de lujo, pero modelándolas de forma diferenciada. La entrada de nuestro modelo consiste en características de la propiedad (superficie construida, dormitorios, baños, cocheras, amenities como gimnasio y pileta), ubicación geográfica (coordenadas, barrio, provincia) e información temporal (mes y año). Utilizamos un enfoque híbrido de dos etapas basado en XGBoost: primero, un clasificador identifica si una propiedad pertenece al segmento de lujo (percentil superior 1.5\%); luego, según la clase asignada, se aplica un regresor especializado para predecir el precio de alquiler mensual en pesos constantes. De esta manera, aprovechamos toda la información disponible sin que el conjunto minoritario de viviendas extremas degrade el desempeño del modelo en el segmento mayoritario. Adicionalmente, empleamos valores SHAP para cuantificar la contribución marginal de cada característica al precio predicho.
