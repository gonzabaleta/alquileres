...
# """
# First preprocessing draft that will only be used in 02-EDA.ipynb
# """
...
class PreprocessingConfig(TypedDict):
    ...
    cap_outliers_cols: NotRequired[
        Dict[str, List]
    ]  # e.g. {"cols": [...], "percentiles": (0.01, 0.99)}
...
def log_transform_features(df: pd.DataFrame, cols: List[str]) -> pd.DataFrame:
    ...
    for col in cols:
        if col in df_processed.columns:
            ...
            print(
                f"Warning: Column '{col}' not found for log transformation. Skipping."
            )
    return df_processed


def cap_outliers(
    df: pd.DataFrame, cols: List[str], percentiles: Tuple[float, float]
...
def apply_capping(
    df: pd.DataFrame, capping_params: Dict[str, Dict[str, float]]
) -> pd.DataFrame:
...
def apply_standardization(
    df: pd.DataFrame, scaling_params: Dict[str, Dict[str, float]]
) -> pd.DataFrame:
...
def preprocess_features(
    ...
    if "log_transform_cols" in config:
        df_processed = log_transform_features(
            df_processed, config["log_transform_cols"]
        )
    if "cap_outliers_cols" in config:
        cap_config = config["cap_outliers_cols"]
        df_processed, capping_params = cap_outliers(
            df_processed, cap_config["cols"], cap_config["percentiles"]
        )
        learned_params["capping"] = capping_params
    if "standardize_cols" in config:
        df_processed, scaling_params = standardize_features(
            df_processed, config["standardize_cols"]
        )
...
def apply_preprocessing(
    df: pd.DataFrame, config: PreprocessingConfig, learned_params: Dict
) -> pd.DataFrame:
